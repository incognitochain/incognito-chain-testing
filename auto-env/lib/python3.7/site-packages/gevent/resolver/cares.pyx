        ...
        if status or not host:
            ...
            try:
                host_result = ares_host_result(host.h_addrtype,
                                               (parse_h_name(host), parse_h_aliases(host), parse_h_addr_list(host)))
            except:
...
cdef class channel:
    ...
    def gethostbyaddr(self, object callback, char*addr):
        ...
        cdef object arg = (self, callback)
        Py_INCREF(arg)
        cares.ares_gethostbyaddr(self.channel, addr_packed, length, family, <void*> gevent_ares_host_callback,
                                 <void*> arg)
