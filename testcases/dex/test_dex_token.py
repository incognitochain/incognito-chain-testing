...
import topology.NodeList_appnet as NodeList
...
class test_dex(unittest.TestCase):
    ...
    def test_DEX01_contribute(self):
        ...
        expecting_797d79_contribution, expecting_562f2b_contribution, refund_797d79, refund_562f2b = \
        ...
        INFO("Expecting 797d79 and 562f2b contribution: %d & %d" % (
            expecting_797d79_contribution, expecting_562f2b_contribution))
        INFO("From API: %d %d %d %d" % (api_contribute_d79, api_contribute_f2b, api_return_d79, api_return_f2b))
        ...
        #     (expecting_797d79_contribution * sum(owner_shareamount_B) / rate_B[0])
        #     + owner_shareamount_B[0]) == owner_shareamount_A[0],
        #             "Contribution shares amount is wrong", "Contribution shares amount is correct")
        assert_true(math.floor(
            ((balance_797d79_B - balance_797d79_A2) * sum(owner_shareamount_B) / rate_B[0])
            + owner_shareamount_B[0]) == owner_shareamount_A[0],
                    "Contribution shares amount is wrong", "Contribution shares amount is correct")
        assert_true(balance_797d79_A2 + balance_797d79_B - balance_797d79_A2 == balance_797d79_B,
                    "Balance 797d79 is wrong, refund is wrong", "Balance 797d79 is correct")
        assert_true(balance_562f2b_A2 + balance_562f2b_B - balance_562f2b_A2 == balance_562f2b_B,
    ...
    def test_DEX01_contribute_revert(self):
        ...
        contribute_token1 = self.fullnode.contribute_token(self.testData['token_ownerPrivateKey'][0],
                                                           self.testData['token_ownerPaymentAddress'][0],
                                                           self.testData['562f2b'],
                                                           self.testData['amount_contribution_562f2b'], "562f2b_797d79")
        ...
        contribute_token2 = self.fullnode.contribute_token(self.testData['token_ownerPrivateKey'][0],
                                                           self.testData['token_ownerPaymentAddress'][0],
                                                           self.testData['797d79'],
                                                           self.testData['amount_contribution_797d79'], "562f2b_797d79")
        INFO("Contribute 797d79 Success, TxID: " + contribute_token2)
        ...
        expecting_562f2b_contribution, expecting_797d79_contribution, refund_562f2b, refund_797d79 = \
            self.cal_actualContribution(self.testData['amount_contribution_562f2b'],
                                        self.testData['amount_contribution_797d79'], rate_B[1], rate_B[0])
        ...
        _, api_contribute_d79, api_return_d79, _, api_contribute_f2b, api_return_f2b = \
            self.fullnode.get_contributionStatus(
                "562f2b_797d79")
        INFO("Contribution amount submitted 562f2b & 797d79: %d & %d" % (self.testData['amount_contribution_562f2b'],
                                                                         self.testData['amount_contribution_797d79']))
        INFO("Expecting 562f2b & 797d79 contribution: %d & %d" % (expecting_562f2b_contribution,
                                                                  expecting_797d79_contribution))
        INFO("From API: %d %d %d %d" % (api_contribute_f2b, api_contribute_d79, api_return_f2b, api_return_d79))
        INFO("Actual 562f2b & 797d79 contribution: %d & %d" % (balance_562f2b_B - balance_562f2b_A2,
                                                               balance_797d79_B - balance_797d79_A2))
        INFO("797d79 balance after contribution (after refund): " + str(balance_797d79_A2))
        INFO("562f2b balance after contribution (after refund): " + str(balance_562f2b_A2))

        assert_true(balance_797d79_A2 + expecting_797d79_contribution == balance_797d79_B,
                    "Balance 797d79 is wrong, refund is wrong", "Balance 797d79 is correct")
        assert_true(balance_562f2b_A2 + expecting_562f2b_contribution == balance_562f2b_B,
                    "Balance 562f2b is wrong, refund is wrong", "Balance 562f2b is correct")
        assert_true(math.floor(
            (expecting_562f2b_contribution * sum(owner_shareamount_B) / rate_B[1])
            + owner_shareamount_B[0]) == owner_shareamount_A[0],
                    "Contribution shares amount is wrong", "Contribution shares amount is correct")
    ...
    def test_DEX02_bulkSwap_nShard(self):
        ...
        for i in range(0, len(self.testData['privateKey'][8])):
            balance_797d79_B.append(self.shard7_trx.get_customTokenBalance(self.testData['privateKey'][8][i],
                                                                           self.testData['797d79'])[0])
            balance_562f2b_temp, _ = self.shard7_trx.get_customTokenBalance(self.testData['privateKey'][8][i],
                                                                            self.testData['562f2b'])
            balance_562f2b_B.append(balance_562f2b_temp)
        ...
        txid_list = []
        for i in range(0, len(privatekey_alias)):
            trade_txid = self.shard7.trade_token(self.testData['privateKey'][8][i],
        ...
        for txid in txid_list:
            print(txid[-6:])
            for i in range(0, 10):
                tx_confirm = self.shard7_trx.get_txbyhash(txid)
                if tx_confirm[0] != "":
                    step3_result = True
                    DEBUG("the " + txid + " is confirmed")
                    break
        ...
        for i in range(0, len(privatekey_alias)):
            tmp_token1Balance_A = False
            for _ in range(0, 10):
                tmp_token1Balance_A, _ = self.shard7_trx.get_customTokenBalance(self.testData['privateKey'][8][i],
                                                                                self.testData['562f2b'])
                if tmp_token1Balance_A > balance_562f2b_B[i]:
                    break
                if i < 3:
            ...
            if tmp_token1Balance_A is not False:
                balance_562f2b_A.append(tmp_token1Balance_A)
                balance_797d79_A.append(self.shard7_trx.get_customTokenBalance(self.testData['privateKey'][8][i],
                                                                               self.testData['797d79'])[0])
            else:
                # ERROR("Wait time expired, 562f2b did NOT increasse")
                assert_true(tmp_token1Balance_A != False, "Wait time expired, 562f2b did NOT increase")
        ...
        for order in sort_order:
            ...
            if received_amount_562f2b == balance_562f2b_A[order] - balance_562f2b_B[order]:
                ...
                result_562f2b.append(str(order) + "Received_False")
                print("Actual: %d" % (balance_562f2b_A[order] - balance_562f2b_B[order]))

            if trade_amount_797d79 == balance_797d79_B[order] - balance_797d79_A[order] - trading_fee[order]:
                result_797d79.append(str(order) + "Trade_True")
            else:
                result_797d79.append(str(order) + "Trade_False")
                print("Actual: %d" % (balance_797d79_B[order] - balance_797d79_A[order] - trading_fee[order]))
    ...
    def test_DEX03_addLiquidity_1shard(self):
        ...
        for i in range(0, len(self.testData['privateKey'][0])):
            ...
            assert_true(balance_562f2b_temp != 0, "commit f2b %d is Zer0" % i)
        ...
        rate_A = self.fullnode.get_latestRate(self.testData["797d79"], self.testData["562f2b"])
        INFO("rate 797d79 vs 562f2b" + str(rate_A))

        # expect_f2b = []
        # expect_d79 = []
        contribution_total_d79 = 0
        contribution_total_f2b = 0
        for i in range(0, len(self.testData['privateKey'][0])):
            ...
            contribution_total_f2b += actual2
        ...
        assert_true(rate_B[1] + contribution_total_f2b == rate_A[1], "Rate f2b after contribution is wrong",
        ...
        #     assert_true(balance_562f2b_B[i] - expect_f2b[i] == balance_562f2b_A[i], "balance f2b of %d is wrong" % i,
