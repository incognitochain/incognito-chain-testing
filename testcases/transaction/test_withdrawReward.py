...
class test_withdrawReward(unittest.TestCase):
    ...
    def test_01_withdraw_reward_PRV(self):
        ...
        INFO("reward : " + str(step2_result[1]))
        assert_true(step2_result[1] != "Unexpected error", " get reward something wrong")

        STEP(3, "withdraw reward")
        step3_result = self.fullnode_tx.withdrawReward(self.test_data["s0_addr1"][0], self.test_data["s0_addr1"][1],self.test_data["token_prv_id"])
        assert_true(step3_result[0] != "Can not send tx", "withdraw reward failed")
        ...
        INFO("add1 balance : " + str(step6_result))
        assert_true(step6_result == step1_result + step2_result[1], "balance receiver not correct")
    ...
    def test_02_withdraw_reward_PRV_0(self):
        ...
        STEP(3, "withdraw reward")
        step3_result = self.fullnode_tx.withdrawReward(self.test_data["s0_addr1"][0], self.test_data["s0_addr1"][1],self.test_data["token_prv_id"])
        assert_true(step3_result[0] == "Can not send tx", "withdraw reward success")
    ...
    def test_03_withdraw_reward_PRV_another_payment_address(self):
        print("""
           ...
           """)
        STEP(1, "get address 1 & 2 balance before sending")
        step1_result_add1 = self.fullnode_tx.getBalance(self.test_data["s0_addr1"][0])
        INFO("addr1_balance: " + str(step1_result_add1))
        ...
        INFO("addr2_balance: " + str(step1_result_add2))
        ...
        STEP(3, "withdraw reward PRV of address2 for payment address 1")
        step3_result = self.fullnode_tx.withdrawReward(self.test_data["s0_addr2"][0], self.test_data["s0_addr1"][1],self.test_data["token_prv_id"])
        assert_true(step3_result[0] != "Can not send tx", "withdraw reward failed")
        ...
        INFO("add2 balance : " + str(step6_result_add2))
    ...
    def test_04_withdraw_reward_pToken(self):
        ...
        STEP(1,"Check balance prv - token address")
        ...
        assert_true(step1_token_reward[0] != "NoToken" or not step1_token_reward[0] != "Unexpected error", "don't have token to withdraw")
        ...
        step2_result = self.fullnode_tx.withdrawReward(self.test_data["s0_addr1"][0], self.test_data["s0_addr1"][1],step1_token_reward[0])
        ...
        step4_token_reward = self.fullnode_tx.check_reward_specific_token(self.test_data["s0_addr1"][1], step1_token_reward[0])
        ...
        assert_true(step4_token_add[0] == step1_token_add[0] + step1_token_reward[1], " get balance prv something wrong")
