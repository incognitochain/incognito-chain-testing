...
def test_send_prv_cross_shard_0_balance_with_privacy_is(fee, privacy):
    ...
    balance1b = sender.get_balance()
    INFO(f"sender balance: {balance1b}")
    assert balance1b == 0, f'Sender balance must = 0 before running this test, got {balance1b} instead' and INFO(
        "Failed")  # add info to debug testcases specific

    balance2b = receiver.get_balance()
