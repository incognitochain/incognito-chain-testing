...
def test_verify_reward_received(dcz):
    ...
    shard_trx_prv_fee_list[0] += COIN_MASTER.send_prv_to(token_holder_shard_0, coin(5), coin(1)).subscribe_transaction().get_fee()
    shard_trx_prv_fee_list[0] += COIN_MASTER.send_prv_to(token_holder_shard_1, coin(5), 100).subscribe_transaction().get_fee()
    shard_trx_prv_fee_list[1] += token_holder_shard_1.send_prv_to(token_holder_shard_0, coin(1), 1000).subscribe_transaction().get_fee()
    shard_trx_prv_fee_list[1] += token_holder_shard_1.send_prv_to(COIN_MASTER, coin(1), 100).subscribe_transaction().get_fee()

    INFO('Create transaction send ptoken')
    if ChainConfig.PRIVACY_VERSION == 1:
        shard_trx_ptoken_fee_list[0] += token_holder_shard_0.send_token_to(token_holder_shard_1, token_id, amount_token_send,
                                                                 token_fee=amount_token_fee).subscribe_transaction().get_privacy_custom_token_fee()
        shard_trx_ptoken_fee_list[0] += token_holder_shard_0.send_token_to(token_holder_shard_1, token_id, amount_token_send,
                                                                 token_fee=amount_token_fee).subscribe_transaction().get_privacy_custom_token_fee()
        shard_trx_ptoken_fee_list[1] += token_holder_shard_1.send_token_to(token_holder_shard_0, token_id, amount_token_send,
                                                                 token_fee=amount_token_fee).subscribe_transaction().get_privacy_custom_token_fee()
        shard_trx_ptoken_fee_list[1] += token_holder_shard_1.send_token_to(token_holder_shard_0, token_id, amount_token_send,
                                                                 token_fee=amount_token_fee).subscribe_transaction().get_privacy_custom_token_fee()
    else:
        shard_trx_prv_fee_list[0] += token_holder_shard_0.send_token_to(token_holder_shard_1, token_id, amount_token_send, prv_fee=amount_token_fee).subscribe_transaction().get_privacy_custom_token_fee()
        shard_trx_prv_fee_list[1] += token_holder_shard_1.send_token_to(token_holder_shard_0, token_id, amount_token_send, prv_fee=amount_token_fee).subscribe_transaction().get_privacy_custom_token_fee()
    ...
    for pub_k, thread in thread_reward_dict.items():
        ...
        reward_PRV[pub_k] = reward.get_result("PRV")
        reward_ptoken[pub_k] = 0 if reward.get_result(token_id) is None else reward.get_result(token_id)
    ...
    for shard, committees in bbd_epoch_reward.get_shard_committees().items():
        ...
        for committee in committees:
            real_earned_reward_b4_prv[shard] += reward_PRV[committee.get_inc_public_key()]
            real_earned_reward_b4_ptoken[shard] += reward_ptoken[committee.get_inc_public_key()]
    ...
    for committee in bbd_epoch_reward.get_beacon_committee():
        real_earned_reward_b4_prv["beacon"] += reward_PRV[committee.get_inc_public_key()]
        real_earned_reward_b4_ptoken["beacon"] += reward_ptoken[committee.get_inc_public_key()]
    ...
    for shard, committees in bbd_epoch_reward.get_shard_committees().items():
        ...
        for committee in committees:
            real_earned_reward_af_prv[shard] += reward_PRV[committee.get_inc_public_key()]
            real_earned_reward_af_ptoken[shard] += reward_ptoken[committee.get_inc_public_key()]

    INFO('Get Beacons earned reward')
    real_earned_reward_af_prv["beacon"] = 0
    real_earned_reward_af_ptoken["beacon"] = 0
    for committee in bbd_epoch_reward.get_beacon_committee():
        real_earned_reward_af_prv["beacon"] += reward_PRV[committee.get_inc_public_key()]
        real_earned_reward_af_ptoken["beacon"] += reward_ptoken[committee.get_inc_public_key()]
