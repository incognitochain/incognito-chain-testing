...
def test_staking(the_stake, validator, receiver_reward, auto_re_stake):
    ...
    bal_before_receiver = receiver_reward.get_prv_balance()
    stake_response = the_stake.stake(validator, receiver_reward, auto_re_stake=auto_re_stake).expect_no_error()
    ...
    if the_stake != receiver_reward:
        assert bal_before_receiver == receiver_reward.get_prv_balance()

    STEP(2, f'Wait until the stake become a committee')
    epoch_plus_n = validator.stk_wait_till_i_am_committee((ChainConfig.BLOCK_PER_EPOCH/2+7)*ChainConfig.BLOCK_TIME)
    ...
    if auto_re_stake:
        result_un_stake = the_stake.stk_un_stake_him(validator).subscribe_transaction()
        un_stake_fee = result_un_stake.get_fee()
        un_stake_block_height = result_un_stake.get_block_height()
        beacon_height = SUT().get_shard_block_by_height(the_stake.calculate_shard_id(), un_stake_block_height).get_beacon_height()
    ...
    the_stake.stk_un_stake_him(validator).expect_error()
    ...
    prv_bal_b4_withdraw_reward = receiver_reward.get_prv_balance()
    prv_reward_amount = receiver_reward.stk_get_reward_amount()
    assert prv_reward_amount > 0, 'User has no PRV reward while expecting some'
    withdraw_fee = receiver_reward.stk_withdraw_reward_to_me().subscribe_transaction().get_fee()
    prv_bal_after_withdraw_reward = receiver_reward. \
    ...
    token_bal_b4_withdraw_reward = receiver_reward.get_token_balance(token_id)
    prv_bal_b4_withdraw_reward = receiver_reward.get_prv_balance()
    token_reward_amount = receiver_reward.stk_get_reward_amount(token_id)
    if ChainConfig.PRIVACY_VERSION == 1:
        ...
        receiver_reward.stk_withdraw_reward_to_me(token_id).subscribe_transaction()
        token_bal_after_withdraw_reward = receiver_reward. \
        ...
        assert prv_bal_b4_withdraw_reward == receiver_reward.get_prv_balance()

    elif ChainConfig.PRIVACY_VERSION == 2:
        assert token_reward_amount == 0, 'Privacy v2 should not have any token reward, PRV only'
        receiver_reward.stk_withdraw_reward_to_me(token_id).expect_error('Not enough reward')
