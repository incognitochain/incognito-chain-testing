...
def test_bulk_swap(test_mode, token_sell, token_buy):
    ...
    balance_tok2_after = []
    private_key_alias = []
    trading_fees = [77, 22, 11, 66, 99, 2, 33, 55, 88, 44]
    ...
    with ThreadPoolExecutor() as executor:
        for i in range(0, len(traders)):
            trader = traders[i]
            future = executor.submit(trader.pde_trade_v2, token_sell, trade_amount, token_buy, 1, trading_fees[i])
    ...
    for i in range(0, len(trading_fees)):
        trade_priority.append(trade_amount_token1 / trading_fees[i])
    ...
    for order in sort_order:
        ...
        bal_tok_2_b4 = balance_tok2_before[order]
        bal_tok_1_b4 = balance_tok1_before[order]
        fee_of_trader_b4 = pde_state_b4.get_trading_fees_amount(trader, token_sell, token_buy)

        bal_tok_2_after = balance_tok2_after[order]
        bal_tok_1_after = balance_tok1_after[order]
        fee_of_trader_after = pde_state_af.get_trading_fees_amount(trader, token_sell, token_buy)

        print(str(order) + "--")
        received_amount_token_buy = calculate_actual_trade_received(trade_amount_token1, calculated_rate[0],
                                                                    calculated_rate[1])
        if received_amount_token_buy == bal_tok_2_after - bal_tok_2_b4 - tx_fee_list[order]:
            result_prv.append(str(order) + "Received_True")
        else:
            result_prv.append(str(order) + "Received_False")
        print("  Actual received: %d" % (bal_tok_2_after - bal_tok_2_b4 - tx_fee_list[order]))

        if trade_amount_token1 == bal_tok_1_b4 - bal_tok_1_after:
            result_token.append(str(order) + "Trade_True")
        else:
            result_token.append(str(order) + "Trade_False")
        print("  Actual Trade amount: %d " % (bal_tok_1_b4 - bal_tok_1_after))

        calculated_rate[1] = calculated_rate[1] - received_amount_token_buy
        # calculated_rate[0] = calculated_rate[0] + trade_amount_token1 + trading_fee[order]
        calculated_rate[0] = calculated_rate[0] + trade_amount_token1
        # if token_sell == PRV_ID:
        #     calculated_rate[0] += trading_fee[order]
        assert fee_of_trader_after - fee_of_trader_b4 == trading_fee
