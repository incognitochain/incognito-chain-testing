...
def test_add_liquidity(contributors, contribute_percent_of_bal_tok2, token1, token2):
    ...
    for acc in contributors:
        ...
        contribute_tx = acc.pde_contribute(token1, commit_amount_tok1[acc], pair_id)
    ...
    for acc in contributors:
        pair_id = pair_ids[acc]
        if token2 is PRV_ID:
            contribute_tx = acc.pde_contribute_prv(commit_amount_tok2[acc], pair_id)
        else:
            contribute_tx = acc.pde_contribute_token(token2, commit_amount_tok2[acc], pair_id)
