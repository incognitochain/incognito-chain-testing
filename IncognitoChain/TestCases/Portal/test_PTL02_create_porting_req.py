...
from IncognitoChain.Configs.Constants import pbnb_id, PortalPortingStatusByPortingId, PortalPortingStatusByTxId, \
    PortalPtokenReqStatus, prv_token_id, coin
...
def test_create_porting_req_1_1(porting_amount, porting_fee, expected):
    ...
    prv_rate = portal_state_info_before_test.get_portal_rate(prv_token_id)
    ...
    if porting_amount == big_porting_amount:
        ...
        # due to big number bug, temporary deposit multiple part
        path = 4
        for i in range(0, path):
            deposit_tx = fat_custodian.make_me_custodian((big_collateral + 1) // path, pbnb_id,
                                                         all_custodians[fat_custodian])
            err = deposit_tx.get_error_msg()
            assert err is None, "Custodian deposit first half fail"
            deposit_tx.subscribe_transaction()

        # create new rate:
        create_rate_tx = PORTAL_FEEDER.create_portal_exchange_rate(big_bnb_rate).subscribe_transaction()
    ...
    if expected == 'valid':
        ...
        for _info in custodian_pool_info_before:
            if _info.get_incognito_addr() == custodian_info.get_incognito_addr():
                ...
                break
        lock_collateral_before = custodian_account.get_my_portal_custodian_status(
        ...
        for _info in custodian_pool_info_after:
            if _info.get_incognito_addr() == custodian_info.get_incognito_addr():
                custodian_account = find_custodian_account_by_incognito_addr(_info.get_incognito_addr())
                break
        lock_collateral_after = custodian_account.get_my_portal_custodian_status(
        ...
        portal_rate_prv = portal_state_info.get_portal_rate(prv_token_id)
...
def test_create_porting_req_1_n():
    ...
    porting_amount = (PortalHelper.cal_portal_exchange_to_token(highest_free_collateral_in_pool.get_free_collateral(),
    ...
    porting_req = portal_user.create_porting_request(pbnb_id, porting_amount)
    porting_id = porting_req.params().get_portal_register_id()
    porting_fee = porting_req.params().get_portal_porting_fee()
    tx_block = porting_req.subscribe_transaction()
    tx_fee = tx_block.get_fee()
...
def test_porting_req_expired():
    ...
    state_before_test = custodian_acc.get_my_portal_custodian_status(portal_state_before_test)
    state_after_expire = custodian_acc.get_my_portal_custodian_status()
