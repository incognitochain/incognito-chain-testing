...
from IncognitoChain.TestCases.Staking_Dynamic_Committee_Size import stake_account, \
    stake_and_check_balance_after_stake, find_public_key_in_beacon_best_state_detail, find_public_key_in_shard_best_state_detail
...
def test_self_stake_and_check_public_key_in_beacon_and_shard_best_state_detail():
    ...
    find_public_key_in_beacon_best_state_detail(stake_account, thread_stake.result()[0], thread_beacon.result())

    STEP(4, 'Find public key in shard 0 best state detail')
    find_public_key_in_shard_best_state_detail(stake_account, thread_stake.result()[1], thread_shard_0.result())

    STEP(5, 'Find public key in shard 1 best state detail')
    find_public_key_in_shard_best_state_detail(stake_account, thread_stake.result()[1], thread_shard_1.result())