...
class BnbCli:
    ...
    def send_bnb_to(self, sender, receiver, amount, password, memo):
        INFO()
        INFO(f'Bnbcli | send {amount} from {l6(sender)} to {l6(receiver)} | memo: {memo}')
        command = [self.cmd, 'send', '--from', sender, '--to', receiver, '--amount', f'{amount}:BNB', '--json',
                   '--memo', memo] + self.get_default_conn()
        return self._exe_bnb_cli(command, password)

    def send_bnb_to_multi(self, sender, receiver_amount_dict: dict, password, memo):
        """
        ...
        """
        INFO()
        INFO(f'Bnbcli | send from {l6(sender)} to {json.dumps(receiver_amount_dict, indent=3)} | memo: {memo}')
        bnb_output = '['
